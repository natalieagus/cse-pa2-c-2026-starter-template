...
#
# Then either:
#   - Pipes the prompt to your configured AI agent via $PA2_AGENT_CMD, or
#   - Prints the prompt to stdout for you to paste into a chat UI manually.
#
# To configure an agent, set PA2_AGENT_CMD before running make:
#
#   export PA2_AGENT_CMD="claude"
#   export PA2_AGENT_CMD="aider --message-file -"
#   export PA2_AGENT_CMD="tee /tmp/prompt.txt"   # save to disk
#
# Without PA2_AGENT_CMD set, the prompt is printed and you paste it
...
for f in "$HEADER" "$SOURCE" "$PROMPT_TEMPLATE" "$AGENTS_DOC"; do
  if [[ ! -f "$f" ]]; then
    ...
    if [[ "$f" == "$HEADER" || "$f" == "$SOURCE" ]]; then
      ...
      echo "If you want tests for the bundled helpers, use MODULE=common." >&2
    fi
    exit 1
  fi
done
...
AGENT_CMD="${PA2_AGENT_CMD:-}"

if [[ -z "$AGENT_CMD" ]]; then
  echo "================================================================" >&2
  echo "PA2_AGENT_CMD not set. Printing prompt to stdout." >&2
